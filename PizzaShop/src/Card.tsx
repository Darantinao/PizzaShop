    ...
    return (
        <>
            <div className='w-auto h-auto rounded-3xl text-2xl p-5 m-5 shadow-lg shadow-gray-700 hover:scale-105 duration-300'>
                ...
                <h1 className='font-bold text-center p-3 '>{recipe.name}</h1>

                <hr className='border-2 border-black mx-1'/>

                <div className='flex flex-wrap pb-1 pt-1 justify-center'>
                    {recipe.tags.map((tag: string, index: number) => (
                        <div key={index} className='border-2 border-black w-fit h-auto m-3 p-1 rounded-xl text-center text-xs/5'>{tag}</div>
                    ))}
                </div>
            ...
            <Overlay isOpen={isOverlayOpen} onClose={() => setOverlayOpen(!isOverlayOpen)}>
                <div className="w-full text-center space-y-3 pt-2">
                    <ol className="font-bold text-3xl">Instructions</ol>
